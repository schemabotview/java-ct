...
**Status:** scaffolded + section spine drafted (**12 modules × ~10 sections = 127 sections**, below). **Modules 01–07 authored end-to-end (the full core-Java half)** — all 76 sections have `.ipynb` + `.slide` + `.tts`, and `manifest.json` wires them onto `java-jvm` / `java-anatomy` (per-section `scene`/`spine`/`focus`/`highlight`, §01 of each the hook). Modules 02–05 ride `java-anatomy`; **modules 06–07 ride `java-jvm`** (07 is the runtime capstone — class loading, GC, execution engine, reflection). Slides are authored to the fuller one-screen density (≥2 sub-sections, code + comparison tables). `audio/` is empty by design: the owner generates the `.wav`s from `tts/` via Colab, then the manifest `audio` fields resolve. **Spring modules 08–12 not authored yet**, but their scene now exists: a dense **`spring`** whole-platform map (5 bands — cloud ▸ web ▸ IoC core ▸ data ▸ boot, one per module) authored + registered app-side in graphl-movie. Scenes `java-jvm`, `java-anatomy`, and `spring` are all registered in graphl-movie.
