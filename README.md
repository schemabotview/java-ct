...
**Status:** scaffolded + section spine drafted (**12 modules × ~10 sections = 127 sections**, below). **Modules 01–03 authored end-to-end** — all 33 sections (11 + 11 + 11) have `.ipynb` + `.slide` + `.tts`, and `manifest.json` wires them onto `java-jvm` / `java-anatomy` (per-section `scene`/`spine`/`focus`/`highlight`, §01 of each the hook). Modules 02–03 are all `java-anatomy`. `audio/` is empty by design: the owner generates the `.wav`s from `tts/` via Colab, then the manifest `audio` fields resolve. Modules 04–12 not authored yet. Scenes `java-jvm` + `java-anatomy` are ported/registered app-side in graphl-movie; Spring modules (08–12) have **no scene yet**.
